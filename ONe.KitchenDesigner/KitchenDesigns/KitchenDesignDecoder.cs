...
{
    public static bool TryDecode(string encoded, out KitchenDesign kitchenDesign)
    {
        try
        {
            ...
            if (version == "1")
            {
                kitchenDesign = LoadV1(encoded);
                return true;
            }
            
            throw new InvalidOperationException("Kitchen design version not recognized.");
        }
        catch (Exception e)
        {
    ...
    private static KitchenDesign LoadV1(string encoded)
    {
